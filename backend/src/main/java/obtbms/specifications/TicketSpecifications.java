    ...
    return (root, query, builder) -> {
      ...
      if (queryType != null) {
        if (queryType.equals("newly_purchased")) {
          predicate = builder.and(predicate, builder.greaterThan(root.get("tripDate"), currentDate));
        } else if (queryType.equals("purchased_successfully")) {
          predicate = builder.and(predicate, builder.lessThanOrEqualTo(root.get("tripDate"), currentDate));
        }
      }
