...
const createUserShema = yup.object({
  firstName: yup
    ...
    .matches(/^[a-zA-ZÀÁÂÃÈÉÊÌÍÒÓÔÕÙÚĂĐĨŨƠàáâãèéêìíòóôõùúăđĩũơƯĂẠẢẤẦẨẪẬẮẰẲẴẶẸẺẼỀỀỂưăạảấầẩẫậắằẳẵặẹẻẽềệểếỄỄỆỈỊỌỎỐỒỔỖỘỚỜỞỠỢỤỦỨỪễỈỊọỏốồổỗộớờởỡợụủứừửữựỳỵỷỹ\s]+$/, 'Tên không được chứa số hoặc ký tự đặc biệt.'),
  lastName: yup
    .string()
    .required('Vui lòng nhập họ.')
    .trim()
    .min(2, 'Họ phải có ít nhất 2 kí tự')
    .matches(/^[a-zA-ZÀÁÂÃÈÉÊÌÍÒÓÔÕÙÚĂĐĨŨƠàáâãèéêìíòóôõùúăđĩũơƯĂẠẢẤẦẨẪẬẮẰẲẴẶẸẺẼỀỀỂưăạảấầẩẫậắằẳẵặẹẻẽềệểếỄỄỆỈỊỌỎỐỒỔỖỘỚỜỞỠỢỤỦỨỪễỈỊọỏốồổỗộớờởỡợụủứừửữựỳỵỷỹ\s]+$/, 'Họ không được chứa số hoặc ký tự đặc biệt.'),
  phone: yup
    .string()
    .trim()
