...
const SideBar = ({ widthSideBar }) => {
  ...
  // Kiểm tra nếu currentUser có dữ liệu và tạo tên đầy đủ
  const userFullName = currentUser ? `${currentUser.lastName} ${currentUser.firstName}` : 'Người dùng';
      ...
      <div className={`h-full`}>
          ...
          <div className="py-4 h-[60px] flex justify-center items-center gap-4 border-b relative ">
            ...
            <h3
              className={`absolute left-13 uppercase font-medium whitespace-nowrap z-10 ${
                widthSideBar === '7%' ? 'invisible opacity-0 transition-all duration-200' : 'opacity-100 transition-all duration-500'
              }`}
            >
              {userFullName}
            </h3>
        ...
        <div className="py-8">
          <Menu
            ...
            mode={widthSideBar !== ADMIN_STYLE.WIDTH_SIDEBAR_CLOSE ? 'inline' : 'vertical'}
            theme="dark"
            items={items}
